...
export const objFields: FieldDescription[] = [
    { name: "event_name", props: { type: "text" } },
    {
        name: "meeting",
    ...
    },
    {
        ...
        label: "none",
        props: { submitText: "UPDATE", cancelText: "CANCEL" },
    },
]

// mix of text and object fields
export const mixedFields: FieldDescription[] = shuffleArray([
    ...textFields,
    ...objFields,
])
...
export const veryCustomizedFields: FieldDescription[] = [
    {
        ...
        props: { prop1: [1, 2, 3], prop2: "foo", prop3: "bar" },
        propsWrapper: { a1: "x", b2: [4, 5, 6], c3: { x: 1, y: 1 } },
    },
    {
        name: "bar",
        component: "CustomField",
        componentWrapper: "CustomWrapper",
        componentFeedback: "CustomFeedback",
        props: { prop1: null, prop2: false, prop3: { x: 1, y: 1 } },
        propsWrapper: { a1: true, b2: [4, 5, 6], c3: "foo" },
    },
]
...
export const validationErrors = {
    name: { short: "Name too short", long: "Name too long" },
    fruits: { many: "Pick 3 fruits at most", few: "Pick 1 fruit at least" },
...
export const validationRules = {
    agree: (val: any) => val,
    name: (val: any) => {
        if (val.length < 3) return validationErrors["name"]["short"]
        if (val.length > 30) return validationErrors["name"]["long"]
        return true
    },
    fruits: (val: any) => {
        if (val.length > 3) return validationErrors["fruits"]["many"]
        if (val.length < 1) return validationErrors["fruits"]["few"]
        return true
    },
    bio: (val: any) => {
        if (!val.includes("hello")) return validationErrors["bio"]
        return true
    },
    languages: (val: any) => {
        if (val.length < 2) return validationErrors["languages"]["few"]
        if (!val.includes("java") && !val.includes("c++"))
            return validationErrors["languages"]["include"]
        return true
    },
    gender: (val: any) => {
        if (val == null || val == "") return validationErrors["gender"]
        return true
    },
}
