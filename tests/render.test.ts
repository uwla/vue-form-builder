...
test("it renders labels", () => {
    ...
    const labels: any = wrapper.findAll(".vfb-group > label")
...
test("it renders input with correct type attributes", () => {
    ...
    const selector =
        "input:not([type=radio]):not([type=checkbox]):not([hidden])"
    const inputs: any = wrapper.findAll(selector)
...
test("it renders checkboxes", () => {
    const checkboxes: any = wrapper.findAll("input[name=fruits]")
    const labels: any = wrapper.findAll("input[name=fruits] ~ label")
...
test("it renders radio", () => {
    const radios: any = wrapper.findAll("input[name=country]")
    const labels: any = wrapper.findAll("input[name=country] ~ label")
...
test("it renders select", () => {
    ...
    // check options
    const options: any = select.findAll("option")
